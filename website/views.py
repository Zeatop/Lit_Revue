    ...
    def get_context_data(self, **kwargs):
        ...
        if user.is_authenticated:
            reviews = Review.objects.filter(
                Q(user__in=user.followed_users.all()) |
                Q(user=user) | 
                Q(ticket__user=user)
                )
            tickets = Ticket.objects.filter(
                Q(user__in=user.followed_users.all()) | Q(user=user)
                )        
...
class ReviewUpdateView(LoginRequiredMixin, UserPassesTestMixin, ActionContextMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['ticket'] = review.ticket  # Ajouter le ticket au contexte
        return context
    ...
    def get_form(self, form_class=None):
        ...
        # Cacher le champ ticket car on ne peut pas le modifier
        if 'ticket' in form.fields:
            form.fields['ticket'].widget = forms.HiddenInput()
        return form
...
class ReviewDeleteView(LoginRequiredMixin, UserPassesTestMixin, ActionContextMixin, DeleteView):
    ...
    model = Review
    template_name = 'website/Review.html'
    success_url = success_url = reverse_lazy('website:review_list')
    action='delete'
    def test_func(self):
        """
